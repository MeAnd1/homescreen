...
function Window({
  ...
  const clamped = useMemo(() => clampDefaults(defaultX, defaultY, defaultWidth, defaultHeight), [defaultX, defaultY, defaultWidth, defaultHeight])
  ...
  const handleMaximize = useCallback(() => {
    ...
    if (isMaximized) {
      ...
      setPreMaximize({ x: rect.left, y: rect.top, width: rect.width, height: rect.height })
      rnd.updatePosition({ x: 0, y: 0 })
      rnd.updateSize({ width: window.innerWidth, height: window.innerHeight - 40 })
      setIsMaximized(true)
    }
